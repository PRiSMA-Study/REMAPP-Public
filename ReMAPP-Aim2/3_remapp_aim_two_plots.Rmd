...
generate_iso_table <- function(df, model_label = "", save_path = NULL) {
  ...
  names(df) <- gsub("^Risk[./]Mscore", "Risk", names(df))   # Risk.Mscore1 → Risk1
  names(df) <- gsub("^N[./]Group",     "N",     names(df))  # N.Group1 → N1 (if ever used)
  names(df) <- gsub("^Thres[./]",     "Thres", names(df))   # Thres/1 → Thres1
...
gt_table <- trimester_summary %>%
  ...
  cols_align(align = "center") %>%              # ← left-align all columns
...
tibble(
    ...
    `Hb (Yes): mean ± SD` = paste0(round(stats_yes$mean_hb, 1), " ± ", round(stats_yes$sd_hb, 2)),
    #`Hb (No): mean ± SD` = paste0(round(stats_no$mean_hb, 1), " ± ", round(stats_no$sd_hb, 2))
  )
}
...
tb_maternal_out_sum <- tb_theme4(maternal_summary_table) %>%
  ...
  tab_footnote(
    footnote = "Hb (Yes): mean ± SD: Hemoglobin concentration (g/dL), reported as mean ± standard deviation separately for outcome = Yes groups.",
    locations = cells_title(groups = "title")
  )
...
⚠ **Note**: Interpretation at high Hb levels (\>15 g/dL) should be cautious due to limited data density.
...
plot_risk_heatmap <- function(grid_df,
  ...
  if (is.null(title)) {
    title <- switch(trimester,
                    ...
                    "Pp6wks" = "Heatmap of Hb Threshold Risk Rank — PostPartum 6wks",
...
plot_all_for_grid <- function(grid, suffix = " ") {
  ...
  p_all <- ggplot(df_new, aes(Hb_sequence, risk, color = Outcome)) +
    ...
    labs(title = paste0("Overview of Risk vs Hb — ", suffix),
  ...
  p6 <- if (nrow(df_fat_score)) {
    ...
    ggplot(df_fat_score, aes(Hb_sequence, score, color = Outcome)) +
      ...
      scale_y_continuous(
      labels = function(x) sub("\\.?0+$","", sprintf("%.2f", x))  # ≤2 decimals, no trailing zeros
      ) +
       x_scale_5_18() +
      theme_bw() +
      theme(
...
```{r all-panel1, fig.width=10, fig.height=8, out.width='80%'}
...
```
\newpage
#### Depression (Standard Cutoff <11) and Fatigue Scores
