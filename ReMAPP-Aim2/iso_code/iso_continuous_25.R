...
t2p_lmer25 <- function(v, n1, n2, tail, covar1=NULL, covar2=NULL, random_effect=NULL) {
...
flexstepreg_lmer25 <- function(y, x, covar1=NULL, covar2=NULL, random_effect=NULL, alpha.adjacency = 1, tail.two = "upper") {
  kk<-0
  ...
  get_numbers_binned <- function(x) {
    ...
    x_binned <- floor(x_clipped * 4 + 0.5) / 4 + 0.125
    
    # Create unique bins and get cumulative counts
    bin_counts <- table(x_binned)
  ...
  binned_result <- get_numbers_binned(x)
  g <- binned_result$g
  x_binned <- binned_result$x_binned
    ...
    for (j in 1:(i - 1)) {
      ...
      while (flag & k <= length(link.rank.score[[j]]$l)) {
        ...
        if (tempN1 < 50 || tempN2 < 50) {
          ...
          p_val <- t2p_lmer25(tempPV, tempN1, tempN2, tail.two, tempCovar1, tempCovar2, tempRandom_effect)
