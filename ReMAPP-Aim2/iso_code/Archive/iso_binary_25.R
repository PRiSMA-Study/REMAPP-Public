...
flexstepreg_glmer_25 <- function(y, x, covar1=NULL, covar2=NULL, random_effect=NULL, alpha.adjacency = 1, tail.two = "upper") {
  ...
  get_numbers_binned <- function(x, bin_width = 0.25) {
    bins <- floor(x / bin_width) * bin_width  # Group into 0.25 intervals
    bin_counts <- rle(sort(bins))$lengths     # Count values in each bin
    c(0, cumsum(bin_counts))                  # Cumulative positions
  }
  
  # Usage:
  g <- get_numbers_binned(x, bin_width = 0.25)
    ...
    for (j in 1:(i - 1)) {
      ...
      while (flag & k <= length(link.rank.score[[j]]$l)) {
        ...
        if (tempN1 < 50 || tempN2 < 50) {
          ...
          if (t2p_glmer(tempPV, tempN1, tempN2, tail.two, tempCovar1, tempCovar2, tempRandom_effect) < alpha.adjacency) {
            flag <- FALSE
