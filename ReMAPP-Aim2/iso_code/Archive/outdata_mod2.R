outdata <- function(data, outcome) {
      ...
      c(
        ...
        if (length(breaks) > 1) {
          sapply(1:(length(breaks)-1), function(i) {
            paste0(">", breaks[1], " - ", "\u2264", breaks[i+1])  # middle ranges
          })
        },
  ...
  for (i in seq_len(nrow(df))) {
    tryCatch({
      result[[paste0("N_Group", i)]] <- df$group_n[i]
      result[[paste0("Risk_Group", i)]] <- round(df$risk_mscore[i], 4)
      result[[paste0("Range_Group", i)]] <- if (i <= length(range_labels)) range_labels[i] else "N/A"
    }, error = function(e) {
      warning("Error assigning values for group ", i, ": ", e$message)
      result[[paste0("N_Group", i)]] <<- NA
      result[[paste0("Risk_Group", i)]] <<- NA
      result[[paste0("Range_Group", i)]] <<- "Error"
    })
  }
