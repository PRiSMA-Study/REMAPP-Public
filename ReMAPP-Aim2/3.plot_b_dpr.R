...
isoplot_dpr <- iso_fun_glmer(df_mat_dpr$hb, df_mat_dpr$dpr, "Likelihood of depression", spline_dpr, iso_dpr)
...
#Output
#****************************************************************************
df_risk <- as.data.frame(iso_dpr$groups) %>% 
  bind_cols(as.data.frame(iso_dpr$estimates)) %>% 
  group_by(`iso_dpr$groups`) %>%  
  summarise(
    group_n = n(),
    risk = sum(`iso_dpr$estimates`)/n())

out_dpr <- as.data.frame(
  list(
    Outcome = "Likelihood of depression",
    N_group1 = df_risk$group_n[1],
    "Risk/Score1" = df_risk$risk[1],
    Threshold1 = iso_dpr$brkPoints[1],
    N_group2 = df_risk$group_n[2],
    "Risk/Score2" = df_risk$risk[2],
    Threshold2 = iso_dpr$brkPoints[2],
    N_group3 = df_risk$group_n[3],
    "Risk/Score3" = df_risk$risk[3],
    Threshold3 = iso_dpr$brkPoints[3],
    N_group4 = df_risk$group_n[4],
    "Risk/Score4" = df_risk$risk[4],
    Threshold4 = iso_dpr$brkPoints[4],
    N_group5 = df_risk$group_n[5],
    "Risk/Score5" = df_risk$risk[5],
    Threshold5 = iso_dpr$brkPoints[5],
    N_group6 = df_risk$group_n[6],
    "Risk/Score6" = df_risk$risk[6],
    Threshold6 = iso_dpr$brkPoints[6],
    N_group7 = df_risk$group_n[7],
    "Risk/Score7" = df_risk$risk[7],
    "Risk/Score7" = df_risk$risk[7],
    Threshold7 = iso_dpr$brkPoints[7],
    N_group8 = df_risk$group_n[8],
    "Risk/Score8" = df_risk$risk[8],
    Threshold8 = iso_dpr$brkPoints[8],
