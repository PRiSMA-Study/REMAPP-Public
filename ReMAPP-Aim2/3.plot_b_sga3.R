...
#Output
#****************************************************************************
df_risk <- as.data.frame(iso_sga3$groups) %>% 
  bind_cols(as.data.frame(iso_sga3$estimates)) %>% 
  group_by(`iso_sga3$groups`) %>%  
  summarise(
    group_n = n(),
    risk = sum(`iso_sga3$estimates`)/n())

out_sga3 <- as.data.frame(
  list(
    Outcome = "SGA (<3th)",
    N_group1 = df_risk$group_n[1],
    "Risk/Score1" = df_risk$risk[1],
    Threshold1 = iso_sga3$brkPoints[1],
    N_group2 = df_risk$group_n[2],
    "Risk/Score2" = df_risk$risk[2],
    Threshold2 = iso_sga3$brkPoints[2],
    N_group3 = df_risk$group_n[3],
    "Risk/Score3" = df_risk$risk[3],
    Threshold3 = iso_sga3$brkPoints[3],
    N_group4 = df_risk$group_n[4],
    "Risk/Score4" = df_risk$risk[4],
    Threshold4 = iso_sga3$brkPoints[4],
    N_group5 = df_risk$group_n[5],
    "Risk/Score5" = df_risk$risk[5],
    Threshold5 = iso_sga3$brkPoints[5],
    N_group6 = df_risk$group_n[6],
    "Risk/Score6" = df_risk$risk[6],
    Threshold6 = iso_sga3$brkPoints[6],
