...
# 2. Categorize outcomes (now case-insensitive)
df_clean <- df_out %>%
  mutate(
    Category = case_when(
      str_detect(Outcome, regex("preterm|lbw|sga|stillbirth|hyperbili|psbi|asph|compo", ignore_case = TRUE)) ~ "Infant Outcomes",
      str_detect(Outcome, regex("pph|ppa|pprom|Depressed|fatigue|epds", ignore_case = TRUE)) ~ "Maternal Outcomes",
      TRUE ~ "Other"
    )
  ) %>%
  #select(Category, Outcome, starts_with("N"), starts_with("Risk"), starts_with("Thres")) %>%
  arrange(Category, desc(Risk1))

# 3. Create formatted table with case-insensitive handling
num_thresholds <- 8  # Subtract 2 for Category and Outcome columns

df_out %>%
  select(Outcome, starts_with("Risk")) %>%
  kbl(caption = "Simplified Risk by Threshold") %>%
  kable_styling("striped", full_width = FALSE) %>%
  column_spec(2:ncol(.), background = "#f7f7f7")
```
\newpage
