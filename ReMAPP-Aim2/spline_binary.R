#****************************************************************************
#spline model - for binary variables
...
knot_fun <- function(data, hb_var, outcome_var){
  ...
  getAIC <- function(nk, model, rm_var, add_var_str){
    ...
    update_frml <- as.formula(sprintf(".~.-%s+%s", rm_var, new_var)) #new var to replace old var
    res <- (update(model, update_frml))$aic #update model with new formula and output aic
    names(res) <- new_var
    return(res)
  }
  ...
  aic_values <- sapply(3:5, getAIC, model = base_model,
                       ...
                       add_var_str = paste0("rcs(", hb_var, ", %d)")) #apply getAIC function on knots 3:5, and the rest 
