...
Note that the criteria are ranked by eligibility percentage, with the most commonly met criteria appearing at the top.
...
The flowchart illustrates the selection process of female participants for inclusion in the Healthy Cohort analysis within the ReMAPP sample, detailing each stage from initial pre-screening to final eligibility determination.

```{r,  echo=FALSE, include=FALSE} 

#This is a the flow chart prep
#Step 1: We want to create subset data for valid hb datasets
 hb_data <- df_hb_wide1_all %>% filter (!is.na(PREG_START_DATE)) %>% 
   mutate (HB_DATA = "Yes")  %>%
 ...
 all_screen_df <- all_screen %>%
   mutate(
     ...
     primary_exclusion = case_when(
       HEALTHY_ELIGIBLE == 1 & screen_pass == "yes" ~ "eligible",
   ...
   left_join(hb_valid_data, by = c("MOMID","PREGID","SITE")) %>% 
    mutate( prescreen_pass = case_when(
           (PRESCREEN == 1 & PRESCR_ELIGIBLE == 1 | (SCREEN == 1 & ELIGIBLE == 1)) ~ "Yes",
            PRESCREEN == 1 & PRESCR_ELIGIBLE %in% c(0,99) ~ "No",
            TRUE ~ NA_character_))
    
  exclusion_counts <- all_screen_df %>%
    filter(HEALTHY_ELIGIBLE == 0) %>%
    ungroup()  %>%
    count(primary_exclusion, name = "count") %>%
    arrange(desc(count))
  
  non_valid_hb <- df_hb_long1_all %>%
    filter(ga_wks < 6 | hb < 5 | hb > 18) %>%
    mutate(sec_exclusion = case_when(
      ga_wks < 6 ~ " gestational age < 6wks",
...
flow <- all_screen_df %>%
  ...
  fc_filter(
    ...
    label_exc = paste(
      ...
      paste0(sum(all_screen$PRESCR_PREGSIGN == 0, na.rm = TRUE), " not pregnant"),
      paste0(sum(all_screen$CONSENT_PRESCREEN == 0, na.rm = TRUE), " did not consent"),
      paste0(sum(all_screen$PRESCR_GA25 == 0, na.rm = TRUE), " GA >= 25wks"),
      paste0(sum(all_screen$PRESCR_CATCHAREA == 0, na.rm = TRUE), " doesn't live in area"),
      paste0(sum(all_screen$PRESCR_AGE == 0, na.rm = TRUE), " doesn't meet age criteria"),
      paste0(sum(all_screen$PRESCR_OTHER == 0, na.rm = TRUE), " other reasons"),
      sep = "\n"
    )
  ) %>%
  ...
  fc_filter(
    ...
    label_exc = paste(
      ...
      paste0(sum(all_screen$GA20 == 0, na.rm = TRUE), " GA >= 20wks"),
      paste0(sum(all_screen$SCRN_RETURN == 0, na.rm = TRUE), " did not return for screening"),
      paste0(sum(all_screen$SCRN_CONSENT == 0, na.rm = TRUE), " withdrew consent"),
      paste0(sum(all_screen$CATCHREMAIN == 0, na.rm = TRUE), " won't remain in area"),
      paste0(sum(all_screen$CATCHAREA == 0, na.rm = TRUE), " doesn't live in area"),
      paste0(sum(all_screen$AGE == 0, na.rm = TRUE), " doesn't meet age criteria"),
      sep = "\n"
    )
  ) %>%
  fc_filter(
...
```
```{r flowchart, out.width='100%', out.height = '85%'} 
...
The table below describes the baseline characteristics and maternal and perinatal outcome data for women enrolled in the Redefining Anemia in Pregnancy and Postpartum (ReMAPP) study compared to those in the healthy cohort.

```{r,  echo=FALSE, include=FALSE} 

#This is a the flow chart prep
#Step 1: We want to create subset data for valid hb datasets
REMAPP <- all_screen_df %>% filter (ELIGIBLE == 1) %>% select(MOMID, PREGID, SITE, HEALTHY_ELIGIBLE, HB_DATA, 
       HB_VALID_DATA) 
...
demo_df <- REMAPP %>% 
  ...
  mutate (
    HEALTHY_CAT = case_when(HEALTHY_ELIGIBLE == 1 & HB_DATA == "Yes" & HB_VALID_DATA == "Yes" ~ "Healthy Cohort Analysis", 
                            TRUE ~"ReMAPP cohort"),
...
demo_table <- demo_df %>%
  ...
  tbl_summary(
    ...
    value = list(
      all_of(dichotomous_vars) ~ "1"  # Only show stats for value = 1
    ),
    label = list(
    ...
    statistic = list(
      ...
      all_categorical() ~ "{p}%"  # Show percentage only
    ),
    digits = list(
      all_categorical() ~ 1  # 1 decimal place
    )
  ) %>%
  modify_header(
  ...
  modify_caption(
    "**<div align='left'><b>Baseline Characteristics and Perinatal Outcomes in Overall ReMAPP Sample vs. Healthy Cohort**\n \n"
  ) %>%
...
```
```{r basetable, out.width='100%', out.height = '85%'} 
...
```{r centile table pooled}
# make centile tables based on the fitted FPR model
# Define a sequence of gestational ages
ga_seq <- seq(12, 40, by = 1)

# Get predicted centiles
centile_table <- centiles(fpr_mod_pooled, 
                          xvar = ga_seq,
                          cent = c(2.5, 5, 10, 50, 90, 95, 97.5), 
                          plot = FALSE)

# Add gestational age to the table
centile_table$gest_age <- ga_seq

# Rearrange columns
centile_table <- centile_table[, c("gest_age", "C2.5", "C5", "C10", "C50", "C90", "C95", "C97.5")]
