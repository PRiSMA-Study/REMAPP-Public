...
clean_date <- function(x, min_year = 2019) {
  ...
  # 🔧 FIX: repair bad 4-digit years like 7023 -> 2023
  x_chr <- stringr::str_replace(
    x_chr,
    "^7(0[0-9]{2}-[0-9]{2}-[0-9]{2})$",  # matches 7023-10-16
    "2\\1"                              # becomes 2023-10-16
  )
  ...
  # Fallback: 5-digit Excel serials stored as text
  ...
  # Drop implausible placeholder years
  yr <- lubridate::year(d)
...
screen_df_clean <- MAT_FLOWCHART %>%
  ...
  #because we do not have a variable to differentiate between remapp prescreened/screened
  #filter out those that might have been prescreened during REMAPP timeline but not screened 
  filter (!(REMAPP_PRESCRN == 1 & REMAPP_SCRN != 1 & ELIGIBLE == 1))  %>%
  #we also want to assume that if screened, then they were prescrenned. also fix timeline issues
...
g6pd_criteria_raw <- mnh08_raw %>%
  ...
  filter(M08_RBC_G6PD_LBORRES > 0 | RBC_G6PD_LBORRES_Interpret %in% c(1:3)) %>%
  ...
  select(SITE, MOMID, PREGID, M08_TYPE_VISIT, M08_RBC_LBPERF_3,
         M08_RBC_G6PD_LBORRES, M08_LBSTDAT, M08_RBC_G6PD_LBORRES_INTERPRET,
         CRIT_G6PD)
...
prep_criteria <- df_maternal %>%
  mutate(
    ...
    TEMP_BMI = case_when (BMI <= 18.5 | BMI >= 30 ~ 0, 
                          BMI > 18.5 & BMI < 30 ~ 1, 
                          TRUE ~ 55),
    BMI_CAT_IMPUT = case_when(
      BMI_IMPUTTED_R < 18.5                        ~ 1,  # Underweight
      BMI_IMPUTTED_R >= 18.5 & BMI_IMPUTTED_R < 25 ~ 2,  # Normal
      BMI_IMPUTTED_R >= 25   & BMI_IMPUTTED_R < 30 ~ 3,  # Overweight
      BMI_IMPUTTED_R >= 30                         ~ 4,  # Obese
      #or if missing use #regular weight
...
crit55_by_site <- df_criteria %>%
  ...
  filter(value == 55) %>%
  count(SITE, CRIT, name = "n_55") %>%
  group_by(SITE) %>%
  mutate(pct_55_within_site = 100 * n_55 / sum(n_55)) %>%
  arrange(SITE, desc(n_55)) %>%
  ungroup()
...
healthyOutcome <- df_criteria %>% 
  ...
  mutate(
    ...
    #With GA Excluded and G6PD Excluded
...
prep_hb1 <- mnh08_raw %>% 
  mutate(M08_LBSTDAT = clean_date (M08_LBSTDAT),
         M08_CBC_HB_LBORRES= if_else(M08_CBC_HB_LBORRES <0, NA, M08_CBC_HB_LBORRES)) %>%
...
df_hb_long1_all <- healthyOutcome %>%
  ...
  mutate(
    visit_date = lubridate::ymd(M08_LBSTDAT),
    ga_wks  = as.numeric(visit_date - lubridate::ymd(PREG_START_DATE)) / 7,
    ga_days = as.numeric(visit_date - lubridate::ymd(PREG_START_DATE)),
...
df_hb_wide1_all <- df_hb_long1_all %>%
  group_by(SITE, MOMID, PREGID) %>%
  summarise(
    PREG_START_DATE       = first(PREG_START_DATE),
    M03_SMOKE_OECOCCUR    = first(M03_SMOKE_OECOCCUR),
    BOE_GA_DAYS_ENROLL    = first(BOE_GA_DAYS_ENROLL),
    HEALTHY_ELIGIBLE      = first(HEALTHY_ELIGIBLE),
    HEALTHY_ELIGIBLE_GA   = first(HEALTHY_ELIGIBLE_GA),
    ...
    hb_list = {
      x <- tibble(visit_date = visit_date, ga_wks = ga_wks, hb = hb) %>%
        filter(!is.na(hb) & !is.na(visit_date)) %>%
        arrange(visit_date)
      
      if (nrow(x) == 0) "No Hb" else
        paste0(
          format(x$visit_date, "%Y-%m-%d"),
          " (", sprintf("%.1f", x$ga_wks), "w): ",
          sprintf("%.1f", x$hb),
          collapse = "; "
        )
    },
...
df_hb_long1_ga <- df_hb_long1_all %>%
  filter (HEALTHY_ELIGIBLE_GA == 1)
...
save(df_hb_long1_ga, file = "derived_data/df_hb_long1_ga.rda")
save(df_hb_wide1_ga, file = "derived_data/df_hb_wide1_ga.rda")
