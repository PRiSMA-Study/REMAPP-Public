...
load("derived_data/df_sec_excl_long.rda")

#include outcome var for df_hb_long1
load("derived_data/df_sensitive_long.rda")
load("derived_data/df_nearmiss_long.rda")
...
#PJW: 5/20/2026 - Commented out because functions not in use
# #test table format 1
# #pvalue format
...
healthy_by_site <- healthyOutcome %>%
  ...
  summarise(
    ...
    "Eligible for healthy cohort (GA + G6PD excluded), n (%)" =
      fmt_np(PRIMARY_HEALTHY_ELIGIBLE),

    # All criteria applied (GA + G6PD applied)
    "Eligible for healthy cohort (all criteria applied), n (%)" =
      fmt_np(PRIMARY_HEALTHY_ELIGIBLE_ALL),

    # excluding infections 
    "Healthy cohort excl. incident infections, n (%)" =
...
row_hc_gapd        <- "Eligible for healthy cohort (GA + G6PD excluded)†, n (%)"
row_hc_all      <- "Eligible for healthy cohort (all criteria applied)‡, n (%)"
row_hc_fpr  <- "Healthy cohort excl. incident infections§, n (%)"
...
tb_data_fixed <- tb_data %>%
  mutate(
    Measure = dplyr::case_when(
      ...
      Measure == "Eligible for healthy cohort (GA + G6PD excluded), n (%)" ~ row_hc_gapd,
      Measure == "Eligible for healthy cohort (all criteria applied), n (%)" ~ row_hc_all,
      Measure == "Healthy cohort excl. incident infections, n (%)" ~ row_hc_fpr,
...
tb1 <- tb_theme2(tb_data2) %>%
  ...
  tab_footnote(
    footnote = "†Primary exclusion criteria for the healthy cohort was applied with the exception of gestational age and G6PD criteria.",
    locations = cells_stub(rows = row_hc_gapd)
  ) %>%
  tab_footnote(
    footnote = "‡All 20 healthy cohort eligibility criteria were applied, including gestational age and exclusion of participants with G6PD deficiency.",
    locations = cells_stub(rows = row_hc_all)
  ) %>%
  tab_footnote(
    footnote = "§Participants with incident infection during pregnancy, including malaria RDT positivity, HIV, hepatitis B, or hepatitis C, were excluded.",
    locations = cells_stub(rows = row_hc_fpr)
  ) %>%
  tab_footnote(
    footnote = "**Outliers removed: gestational age < 5 or > 42 weeks, or hemoglobin < 5 or > 18 g/dL."
  )
...
<!-- #     footnote = c("**Outliers: GA < 5 or GA < 42 or HB < 5 or HB > 18") -->
 ...
 all_screen_df <- all_screen %>%
   mutate(
     ...
     primary_exclusion = case_when(
       PRIMARY_HEALTHY_ELIGIBLE == 1 & SCREEN_PASS == "Yes" & PRISMA_ENROLL == 1 ~ "eligible",
  ...
  exclusion_counts <- all_screen_df %>%
    filter(PRIMARY_HEALTHY_ELIGIBLE == 0 & PRISMA_ENROLL == 1 & SCREEN_PASS == "Yes") %>%
    ungroup()  %>%
...
## 3: Further exclusions exclusions ----
step4_df <- all_screen_df %>%
  # keep only rows that have a screening exclusion
  filter(PRIMARY_HEALTHY_ELIGIBLE == 1 & SECL_EXCLU == 1 & HEALTHY_ELIGIBLE_G6PD == 0) %>%   # drop those who passed screening
  count(reason = SECL_EXCLU_RSN_LABEL, name = "n")

step4_exc_lines <- step4_df %>%
  filter(n > 0) %>%
  arrange(desc(n)) %>%
  mutate(line = paste0(n, " ", reason)) %>%
  pull(line)

print(step4_exc_lines)
...
flow <- all_screen_df %>%
  ...
  # Screening step
  fc_filter(
    SCREEN_PASS == "Yes" & PRESCREEN_PASS == "Yes" & PRISMA_ENROLL == 1,
  ...
  fc_filter(
    PRIMARY_HEALTHY_ELIGIBLE == 1 & PRISMA_ENROLL == 1,
    label = "Women who met the healthy cohort criteria",
    text_pattern = "{label}\n n={n}",
    text_color = "black",
    bg_fill = "#ebfcf9",
    border_color = "#016795",
  ...
  fc_filter(
    HEALTHY_ELIGIBLE_G6PD == 1 & PRISMA_ENROLL == 1 & SECL_EXCLU == 0,
    label = paste0(
      "Women included in FPR analysis with at least\none valid hb value\n"),
    ...
    offset_exc = 0.08,
    label_exc        = paste(
      "Excluded incident infection:",
      paste(step4_exc_lines, collapse = "\n"),
      sep = "\n"
    )
  ) 
...
REMAPP <- all_screen %>% 
  select(SITE, MOMID, PREGID, SCREEN_PASS, PRISMA_ENROLL, HEALTHY_ELIGIBLE_G6PD)  %>%
  filter(SCREEN_PASS == "Yes" & PRISMA_ENROLL == 1) %>%
...
hc_anemia_who_disp <- hc_anemia_who %>%
  ...
  dplyr::left_join(
    hb_summary_median_disp %>%
      dplyr::select(`Time period`, `GA weeks`, `95th centile (g/dL)`),
    by = c("Time period", "GA weeks")
  )
...
### Excluding the subgroup of subjects with adverse oucomes - preterm delivery
...
fpr_mod_sensitive_fun <- function(data, model, s, label){
...
fpr_mod_sensitive_pooled <- fpr_mod_sensitive_fun(df_fpr, fpr_mod_pooled, "preterm37_mom", "preterm birth <37weeks")
...
### Exclusion of participants with severe maternal morbidity 

To assess whether participants with severe maternal morbidity have a different hemoglobin distribution, we compared hemoglobin centile curves in subsamples excluding these participants (red line) with those from the full enrolled cohort (black line).

Sensitivity analysis excluding severe maternal morbidities including severe preeclampsia, severe postpartum hemorrhage, organ failure, or ICU admission

```{r , results='hide', fig.align='center', out.width='60%'}
# centiles
cents <- c(5, 50, 95)

df_fpr_sev <- df_nearmiss_long %>% 
dplyr :: select(SITE, ga_wks, hb, near_miss) %>% 
  mutate(nearmiss = ifelse(!is.na(near_miss), near_miss, 55)) %>%
  filter(!is.na(hb) & !is.na(ga_wks)) %>% 
  mutate(range = (max(ga_wks) - min(ga_wks)))
...
fpr_mod_mat_sensitive_pooled <- fpr_mod_sensitive_fun(df_fpr_sev, fpr_mod_pooled, "nearmiss","severe maternal morbidities")
